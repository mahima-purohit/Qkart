...
const Checkout = () => {
  ...
  // Items contain the cart data from generateCarData()
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      // console.log(newAddress.value,"newAddress.val")
      // console.log(config.endpoint,"config.endpoint");
      const addAddressResponse = await axios.post(`${config.endpoint}/user/addresses`,
      {
      ...
      // console.log(addAddressResponse.data,"addAddressResponse.data");
      setAddresses({ ...addresses, all:addAddressResponse.data });
  ...
  const handleAddressSelect = async (myAddress) =>{
    // console.log("handleAddressSelect is called");
    console.log(myAddress,"myAddress");
    setAddresses({
      ...addresses, selected:myAddress._id
    })
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      // console.log("deleteAddressIsCalled");
      const deleteAddressResponse = await axios.delete(`${config.endpoint}/user/addresses/${addressId}`,
      {
        headers: 
        {
          'Authorization': `Bearer ${token}`
      ...
      setAddresses({ ...addresses, all:deleteAddressResponse.data });
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const validateRequest = async (items, addresses) => { 
    let validateRequestFlag = true;
    const finalAmount = getTotalCartValue(items);
    const balance = localStorage.getItem("balance");
    if(finalAmount>balance){
      enqueueSnackbar("You do not have enough balance",{variant:"warning"})
      return validateRequestFlag=false;
    }
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
        const performCheckoutPostResponse = await axios.post(`${config.endpoint}/cart/checkout`,
        {
      ...
      // console.log(performCheckoutPostResponse,"performCheckoutPostResponse");
      let remainingBalanace = localStorage.getItem("balanace")-getTotalCartValue(items);
      localStorage.setItem("balance",remainingBalanace);
      history.push("/thanks");
      enqueueSnackbar("Order placed successfully ",{variant:"success"});
      }
      catch(e){
        if(e.response && e.response.message){
          const errrorResponse = await e.response.message;
          console.log(errrorResponse,"response from error");
          enqueueSnackbar(errrorResponse,{variant:"warning"});
        }
      }
  };
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      const responseFromGetAddresses = await getAddresses(token);
      // console.log(responseFromGetAddresses,"responseFromGetAddresses");
    };
    onLoadHandler();
    ...
    <>
      ...
      <Grid container>
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {(addresses.all.length>0)&&
                addresses.all.map((myAddress)=>{
                  return(
                      <Box key={myAddress._id}
                      className={`address-item ${addresses.selected === myAddress._id ? 'selected': ''}`}
                      onClick={async ()=>await handleAddressSelect(myAddress)}
                      >
                             {myAddress.address}
                        <Button startIcon={<DeleteIcon/>}
                                onClick={async ()=>await deleteAddress(token,myAddress._id)}
                        >
                            DELETE
                        </Button>
                      </Box>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          <Cart isReadOnly={true} isCheckout products={products} items={items} />
        </Grid>
      </Grid>
      <Footer />
    </>
